      ...
      <div class="container mx-auto px-4 py-3 flex justify-between items-center">
        ...
        <div class="flex items-center space-x-3">
          <div class="flex items-center bg-white/10 rounded-lg p-2">
            ...
            <h1 class="ml-2 text-xl font-bold tracking-tight">LutzPayroll.inc</h1>
          </div>
          <!-- UTC Time Display -->
          <div class="hidden md:flex items-center bg-white/5 rounded-lg px-3 py-1.5">
            <span class="material-icons text-sm mr-2">schedule</span>
            <time class="text-sm font-medium">{{currentDateTime}}</time>
          </div>
        </div>
  
        <!-- User Controls -->
        <div class="flex items-center space-x-4">
          <div class="flex items-center bg-white/5 rounded-lg p-2 hover:bg-white/10 transition-all">
            <div class="h-9 w-9 rounded-full bg-gradient-to-br from-blue-400 to-indigo-400 flex items-center justify-center shadow-inner">
              <span class="text-lg font-semibold">{{userInitial}}</span>
            </div>
          ...
          <button @click="logout" 
                  class="flex items-center px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20 
                         transition-all duration-200 focus:outline-none focus:ring-2 
                         focus:ring-white/50 active:scale-95">
            <span class="material-icons text-sm">logout</span>
            <span class="ml-2 text-sm font-medium hidden md:inline">Logout</span>
          </button>
        </div>
      </div>
    </header>
