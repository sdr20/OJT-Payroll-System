<template>
  <div class="min-h-screen bg-gray-50 p-1">
    <div class="max-w-8xl mx-auto space-y-6">
          ...
          <table class="w-full">
            <thead>
              <tr class="bg-gray-50 border-b border-gray-100">
                <th class="px-6 py-4 text-sm font-semibold text-gray-600 tracking-wider">Employee ID</th>
                ...
                <th class="px-6 py-4 text-sm font-semibold text-gray-600 tracking-wider text-right">Actions</th>
              </tr>
            </thead>
      ...
      <div v-if="showRecordsModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
          <div class="p-6 border-b flex justify-between items-center">
            <h2 class="text-2xl font-semibold text-gray-800">Financial Records for {{ selectedEmployee.firstName }} {{ selectedEmployee.lastName }}</h2>
            <button
          ...
          <div class="p-6">
            <div class="grid grid-cols-1 gap-6">
              <div>
                <h3 class="text-lg font-semibold text-gray-800 mb-4">Monthly Financial Contributions and Salary Dates</h3>
                <table class="w-full text-left border">
                  <thead class="bg-gray-100">
                    <tr>
                      <th class="px-4 py-2 text-sm font-medium text-gray-700">Month</th>
                      <th class="px-4 py-2 text-sm font-medium text-gray-700">SSS Contribution</th>
                      <th class="px-4 py-2 text-sm font-medium text-gray-700">PhilHealth Contribution</th>
                      <th class="px-4 py-2 text-sm font-medium text-gray-700">Pag-IBIG Contribution</th>
                      <th class="px-4 py-2 text-sm font-medium text-gray-700">Withholding Tax</th>
                      <th class="px-4 py-2 text-sm font-medium text-gray-700">Expected Salary Date</th>
                    </tr>
                  </thead>
                  <tbody class="divide-y divide-gray-200">
                    <tr v-for="record in contributionRecords" :key="record.month">
                      <td class="px-4 py-2 text-sm text-gray-900">{{ record.month }}</td>
                      <td class="px-4 py-2 text-sm text-gray-900">₱{{ record.sss.toLocaleString() }}</td>
                      <td class="px-4 py-2 text-sm text-gray-900">₱{{ record.philhealth.toLocaleString() }}</td>
                      <td class="px-4 py-2 text-sm text-gray-900">₱{{ record.pagibig.toLocaleString() }}</td>
                      <td class="px-4 py-2 text-sm text-gray-900">₱{{ record.withholdingTax.toLocaleString() }}</td>
                      <td class="px-4 py-2 text-sm text-gray-900">{{ record.expectedSalaryDate }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
      ...
      <!-- PDF Preview Modal -->
      <div v-if="showPDFPreviewModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
          <div class="p-6 border-b flex justify-between items-center">
            <h2 class="text-2xl font-semibold text-gray-800">PDF Preview for {{ selectedEmployee.firstName }} {{ selectedEmployee.lastName }}</h2>
            <button
...
export default {
  ...
  methods: {
    ...
    generateContributionRecords(employee) {
      ...
      const currentDate = new Date('2025-03-04'); // Current date as per system instructions
