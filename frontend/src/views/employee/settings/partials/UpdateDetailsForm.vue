...
const fetchPositions = async () => {
    try {
        ...
        updateMessage.value = 'Failed to load positions. Using default options.';
        positions.value = [
            { _id: '1', name: 'Software Engineer' },
            { _id: '2', name: 'Developer' },
            { _id: '3', name: 'Project Manager' },
            { _id: '4', name: 'HR Specialist' },
            { _id: '4', name: 'System Administrator' },
        ];
    }
};
...
watch(() => props.employee, (employee) => {
    if (employee) {
        form.value = {
            ...
            position: employee.position?._id || employee.position || '', // Use ObjectId if populated, otherwise raw ID
...
const resetForm = () => {
    if (props.employee) {
        form.value = {
            ...
            position: props.employee.position?._id || props.employee.position || '',
...
const updateEmployee = async () => {
    ...
    try {
        ...
        const response = await fetch(`${BASE_API_URL}/api/employee/update/${props.employee._id}`, { // Fixed to /employee
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`,
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select v-model="form.position" id="position"
                        class="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                        <option value="" :selected="!props.employee?.position">Select a position</option>
                        <option v-for="pos in positions" :key="pos._id" :value="pos._id"
                            :selected="pos._id === (props.employee?.position?._id || props.employee?.position)">
                            {{ pos.name }}
                        </option>
                    </select>
                </div>
