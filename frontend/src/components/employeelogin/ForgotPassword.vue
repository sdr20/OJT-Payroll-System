      ...
      <div class="w-full max-w-md">
        <div class="bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl p-8 space-y-6">
          <div class="text-center space-y-2">
            ...
            <p class="text-gray-500 text-sm">{{ step === 1 ? 'Enter your email to receive a verification code' : 'Enter the code and your new password' }}</p>
          </div>
          ...
          <form v-if="step === 1" @submit.prevent="sendVerificationCode" class="space-y-4">
            ...
            <div class="text-center">
              <router-link to="/employee-login" class="text-sm text-blue-500 hover:underline">Back to Login</router-link>
            </div>
          </form>
  ...
  export default {
    ...
    methods: {
      ...
      async resetPassword() {
        ...
        this.error = '';
        try {
          await axios.post('http://localhost:7777/api/auth/reset-password', {
            email: this.email,
