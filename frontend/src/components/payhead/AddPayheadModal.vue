    ...
    <div class="bg-white p-6 rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
      ...
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-2xl font-bold text-gray-800">
          {{ mode === 'bulkAssign' ? 'Bulk Assign Payheads to Employees' : (isUpdate ? 'Edit Payheads' : 'Add Payheads for Employee') }}
        </h2>
      ...
      <div v-if="mode === 'bulkAssign'" class="space-y-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="space-y-2">
            ...
            <div class="max-h-64 overflow-y-auto border rounded-lg p-2">
              ...
              <div v-else v-for="payhead in availablePayheads" :key="payhead.id" class="flex items-center p-2 hover:bg-gray-50 rounded">
                <input
                  type="checkbox"
                  :value="payhead"
                  v-model="selectedPayheads"
                  class="mr-2 h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                />
                <label class="text-sm text-gray-700">
                  {{ payhead.name }} ({{ payhead.type }}, ₱{{ payhead.amount.toLocaleString() }}{{ payhead.isRecurring ? ', Recurring' : '' }})
                </label>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-2">
            ...
            <div class="max-h-64 overflow-y-auto border rounded-lg p-2">
              ...
              <div v-else v-for="employee in employees" :key="employee.id" class="flex items-center p-2 hover:bg-gray-50 rounded">
                <input
                  type="checkbox"
                  :value="employee"
        ...
        <div class="mt-6 flex justify-end gap-4">
          ...
          >
            <span class="material-icons-outlined text-base">save</span>Assign Payheads
          </button>
        </div>
      </div>
      ...
      <div v-else class="space-y-4">
        ...
        <div class="space-y-2" v-if="!isUpdate">
          ...
          <button
            ...
            title="Add Payhead"
          >
            <span class="material-icons-outlined text-base">add</span>Add
          </button>
        </div>
      ...
      <div v-if="!mode || mode !== 'bulkAssign'" class="mt-6 flex justify-end gap-4">
        <button
          ...
          title="Cancel"
        >
          <span class="material-icons-outlined text-base">close</span>Close
        </button>
        <button
          @click="$emit('save')"
          class="py-2 px-4 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition duration-200 flex items-center gap-1"
